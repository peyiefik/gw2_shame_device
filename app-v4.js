(() => {
  ...
  const els = {
    ...
    supportReviewSection: document.getElementById("support-review-section"),
    supportReviewBody: document.getElementById("support-review-body"),
    supportReviewCount: document.getElementById("support-review-count"),
  };
    ...
    els.awards.innerHTML = (awards || []).map((item, index) => {
      ...
      return `<article class="award-card${item.featured || index === 0 ? " featured" : ""}">
        ...
        <div>
          ...
          ${els.showAccounts.checked && p?.account && p.account !== p.displayName ? `<div class="award-account">${escapeHtml(p.account)}</div>` : ""}
  ...
  function renderSupportReview() {
    const rows = state.supportOutliers || [];
    if (!els.supportReviewSection || !els.supportReviewBody) return;

    if (!rows.length) {
      els.supportReviewSection.hidden = true;
      els.supportReviewBody.innerHTML = "";
      if (els.supportReviewCount) els.supportReviewCount.textContent = "";
      return;
    }

    els.supportReviewSection.hidden = false;
    if (els.supportReviewCount) {
      els.supportReviewCount.textContent = `${rows.length} suspicious one-off${rows.length === 1 ? "" : "s"}`;
    }

    const showAccounts = els.showAccounts.checked;
    els.supportReviewBody.innerHTML = rows.map((point) => {
      const checked = !point.excluded;
      const provider = `<span class="player-name">${escapeHtml(point.displayName || point.account)}</span>${showAccounts && point.account && point.account !== point.displayName ? `<span class="player-account">${escapeHtml(point.account)}</span>` : ""}`;
      return `<tr>
        <td><input class="support-point-toggle" type="checkbox" data-support-key="${escapeHtml(point.key)}" ${checked ? "checked" : ""} aria-label="Use this ${escapeHtml(point.boon)} datapoint" /></td>
        <td>${provider}</td>
        <td>${escapeHtml(point.boon)}</td>
        ...
        <td class="bad-number">${safeNumber(point.uptime).toFixed(1)}%</td>
        <td>${safeNumber(point.usual).toFixed(1)}%</td>
        <td class="bad-number">−${safeNumber(point.gap).toFixed(1)} pp</td>
      </tr>`;
    }).join("");
  }
  ...
  function installWorker() {
    ...
    state.worker = new Worker("worker-v4.js?v=1");
    ...
    state.worker.onmessage = (event) => {
      ...
      if (msg.type === "result") {
        ...
        state.supportOutliers = Array.isArray(msg.supportOutliers) ? msg.supportOutliers : [];
        renderAll();
        setBusy(false);
        setStatus(`Done: ${state.logs.length} usable logs, ${state.players.length} players${state.errors.length ? `, ${state.errors.length} failed` : ""}${state.supportOutliers.length ? ` · ${state.supportOutliers.length} support datapoint${state.supportOutliers.length === 1 ? "" : "s"} flagged for review` : ""}.`, "success");
  ...
  function analyze() {
    ...
    state.excludedSupportKeys = new Set();
    state.supportOutliers = [];
    installWorker();
    setBusy(true);
  ...
  function loadDemo() {
    ...
    state.awards = [
      ...
      { icon: "⏩", title: "Quickness Outage", winner: players[2], stat: "87.4% average subgroup Quickness · provider average 94.6% · 3 included Quickness logs", roast: "Lowest included Quickness uptime among detected providers — 7.2 percentage points below the provider average." },
      { icon: "⏱️", title: "Alacrity Outage", winner: players[3], stat: "89.2% average subgroup Alacrity · provider average 96.1% · 3 included Alacrity logs", roast: "Lowest included Alacrity uptime among detected providers — 6.9 percentage points below the provider average." },
    ...
    state.supportOutliers = [
      {
        key: "Quickness|Hammer.5555|https://dps.report/demo-gors",
        account: "Hammer.5555",
        displayName: "Bonk Department",
        boon: "Quickness",
        url: "https://dps.report/demo-gors",
        fightName: "Gorseval",
        uptime: 51.2,
        usual: 94.8,
        gap: 43.6,
    ...
    state.errors = [];
    renderAll();
    setStatus("Demo raid night loaded. One suspicious support datapoint is available for review.", "success");
  ...
  els.includeWipes.addEventListener("change", () => {
    ...
    state.worker.postMessage({
      ...
      excludedSupportKeys: [...state.excludedSupportKeys],
    });
  });
  ...
  if (els.supportReviewBody) {
    els.supportReviewBody.addEventListener("change", (event) => {
      const target = event.target;
      if (!(target instanceof HTMLInputElement) || !target.classList.contains("support-point-toggle")) return;
      const key = target.dataset.supportKey;
      if (!key) return;

      if (target.checked) state.excludedSupportKeys.delete(key);
      else state.excludedSupportKeys.add(key);
      ...
      setStatus("Recalculating support uptime with your datapoint choice…", "loading");
      state.worker.postMessage({
        type: "recalculate",
        includeWipes: els.includeWipes.checked,
        excludedSupportKeys: [...state.excludedSupportKeys],
