...
function boonUptime(player, buffId) {
  ...
  for (const entries of collections) {
    ...
    for (const entry of entries) {
      if (n(entry?.id, -1) !== buffId) continue;
      const uptime = Number(first(entry?.buffData)?.uptime);
      if (!Number.isFinite(uptime)) return 0;
      return Math.max(0, Math.min(100, uptime));
    }
  }

  return sawCollection ? 0 : null;
}

function subgroupCoverage(players, provider, uptimeKey, durationMS) {
  const sameGroup = players.filter((member) =>
  ...
  const recipients = sameGroup.length
    ? sameGroup
  ...
  if (!recipients.length) return { uptime: null, weight: 0, recipients: 0 };

  let weighted = 0;
  let weight = 0;
  for (const member of recipients) {
    const memberWeight = member.activeMs > 0 ? member.activeMs : Math.max(1, n(durationMS, 1));
    weighted += n(member[uptimeKey]) * memberWeight;
    weight += memberWeight;
  }
...
function compactLog(json, url) {
  ...
  for (const p of rawPlayers) {
    ...
    const quicknessUptime = boonUptime(p, QUICKNESS_ID);
    const alacrityUptime = boonUptime(p, ALACRITY_ID);
    ...
    if (!cp) {
      cp = {
        ...
        quicknessSelfUptime: quicknessUptime,
        alacritySelfUptime: alacrityUptime,
        quicknessCoverage: null,
        quicknessCoverageWeight: 0,
        alacrityCoverage: null,
        alacrityCoverageWeight: 0,
    ...
    if (quicknessUptime !== null) cp.quicknessSelfUptime = quicknessUptime;
    if (alacrityUptime !== null) cp.alacritySelfUptime = alacrityUptime;
  ...
  const compactPlayers = [...byAccount.values()];
  for (const cp of compactPlayers) {
    ...
    if (cp.providesQuickness) {
      const coverage = subgroupCoverage(compactPlayers, cp, "quicknessSelfUptime", json?.durationMS);
      cp.quicknessCoverage = coverage.uptime;
      cp.quicknessCoverageWeight = coverage.weight;
    }
    if (cp.providesAlacrity) {
      const coverage = subgroupCoverage(compactPlayers, cp, "alacritySelfUptime", json?.durationMS);
      cp.alacrityCoverage = coverage.uptime;
      cp.alacrityCoverageWeight = coverage.weight;
    }
  }
...
function emptyAggregate(p) {
  return {
    ...
    quicknessGenerationMax: 0,
    alacrityGenerationMax: 0,
    quicknessCoverageWeighted: 0,
    quicknessCoverageWeight: 0,
    alacrityCoverageWeighted: 0,
    alacrityCoverageWeight: 0,
    quicknessUptime: 0,
    alacrityUptime: 0,
    providesQuickness: false,
...
function aggregate(logs, includeWipes) {
  ...
  for (const log of included) {
    for (const p of log.players) {
      ...
      if (p.providesQuickness) {
        a.quicknessProviderLogs += 1;
        if (p.quicknessCoverage !== null && p.quicknessCoverageWeight > 0) {
          a.quicknessCoverageWeighted += n(p.quicknessCoverage) * p.quicknessCoverageWeight;
          a.quicknessCoverageWeight += p.quicknessCoverageWeight;
        }
      }
      if (p.providesAlacrity) {
        a.alacrityProviderLogs += 1;
        if (p.alacrityCoverage !== null && p.alacrityCoverageWeight > 0) {
          a.alacrityCoverageWeighted += n(p.alacrityCoverage) * p.alacrityCoverageWeight;
          a.alacrityCoverageWeight += p.alacrityCoverageWeight;
        }
      }
  ...
  for (const p of players) {
    ...
    p.quicknessUptime = p.quicknessCoverageWeight ? p.quicknessCoverageWeighted / p.quicknessCoverageWeight : 0;
    p.alacrityUptime = p.alacrityCoverageWeight ? p.alacrityCoverageWeighted / p.alacrityCoverageWeight : 0;
...
function weightedProviderAverage(providers, uptimeKey, weightKey) {
  let total = 0;
  let weight = 0;
  for (const provider of providers) {
    const w = n(provider[weightKey]);
    if (w <= 0) continue;
    total += n(provider[uptimeKey]) * w;
    weight += w;
  }
...
function buildAwards(players) {
  ...
  const worstQuick = bottom(quickProviders, "quicknessUptime");
  if (worstQuick) {
    const providerAverage = weightedProviderAverage(quickProviders, "quicknessUptime", "quicknessCoverageWeight");
    const gap = Math.max(0, providerAverage - worstQuick.quicknessUptime);
    awards.push(award(
      "quickness-uptime", "⏩", "Quickness Outage", worstQuick,
      `${worstQuick.quicknessUptime.toFixed(1)}% average subgroup Quickness · provider average ${providerAverage.toFixed(1)}% · ${plural(worstQuick.quicknessProviderLogs, "Quickness log")}`,
      `Lowest active-time-weighted Quickness uptime among detected Quickness providers${gap >= 0.1 ? ` — ${gap.toFixed(1)} percentage points below the provider average.` : "."}`
    ));
  }

  const alacProviders = players.filter((p) => p.alacrityProviderLogs > 0 && p.alacrityCoverageWeight > 0);
  const worstAlac = bottom(alacProviders, "alacrityUptime");
  if (worstAlac) {
    const providerAverage = weightedProviderAverage(alacProviders, "alacrityUptime", "alacrityCoverageWeight");
    const gap = Math.max(0, providerAverage - worstAlac.alacrityUptime);
    awards.push(award(
      "alacrity-uptime", "⏱️", "Alacrity Outage", worstAlac,
      `${worstAlac.alacrityUptime.toFixed(1)}% average subgroup Alacrity · provider average ${providerAverage.toFixed(1)}% · ${plural(worstAlac.alacrityProviderLogs, "Alacrity log")}`,
      `Lowest active-time-weighted Alacrity uptime among detected Alacrity providers${gap >= 0.1 ? ` — ${gap.toFixed(1)} percentage points below the provider average.` : "."}`
    ));
  }
