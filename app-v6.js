(() => {
  ...
  const els = {
    ...
    boonEstimateSection: document.getElementById("boon-estimate-section"),
    boonEstimateBody: document.getElementById("boon-estimate-body"),
    boonEstimateCount: document.getElementById("boon-estimate-count"),
  };
  ...
  function renderReview() {
    if (!els.reviewSection || !els.reviewBody) return;
    const rows = state.reviewPoints || [];

    if (!rows.length) {
      els.reviewSection.hidden = true;
      els.reviewBody.innerHTML = "";
      if (els.reviewCount) els.reviewCount.textContent = "";
      return;
    }
    ...
    if (els.reviewCount) {
      ...
      els.reviewCount.textContent = `${rows.length} flagged${excluded ? ` · ${excluded} excluded` : ""}`;
    }

    const showAccounts = els.showAccounts.checked;
    els.reviewBody.innerHTML = rows.map((point) => {
      const checked = !point.excluded;
      const player = `<span class="player-name">${escapeHtml(point.displayName || point.account)}</span>${showAccounts && point.account && point.account !== point.displayName ? `<span class="player-account">${escapeHtml(point.account)}</span>` : ""}`;
      const direction = point.direction === "low" ? "unusually low" : "unusually high";
      return `<tr>
        <td><input class="review-point-toggle" type="checkbox" data-point-key="${escapeHtml(point.key)}" ${checked ? "checked" : ""} aria-label="Use this ${escapeHtml(point.metricLabel)} datapoint" /></td>
        <td>${player}</td>
        <td><strong>${escapeHtml(point.metricLabel)}</strong><br><span class="muted-note">${escapeHtml(direction)}</span></td>
        <td><a class="encounter-link" href="${escapeHtml(point.url)}" target="_blank" rel="noopener noreferrer">${escapeHtml(point.fightName)} ↗</a></td>
        <td class="bad-number">${escapeHtml(point.valueText)}</td>
        <td>${escapeHtml(point.usualText)}</td>
        <td class="bad-number">${escapeHtml(point.gapText)}</td>
      </tr>`;
    }).join("");
  }

  function providerOptions(items, boon) {
    if (!Array.isArray(items) || !items.length) return "none detected";
    const showAccounts = els.showAccounts.checked;

    return items.map((p) => {
      const group = safeNumber(p.group) > 0 ? `G${safeNumber(p.group)}` : "group ?";
      const checked = p.selected ? "checked" : "";
      const inferred = p.inferred ? " · inferred" : " · incidental?";
      const account = showAccounts && p.account && p.account !== p.displayName
        ? `<span class="player-account">${escapeHtml(p.account)}</span>`
        : "";
      return `<label class="toggle" style="margin:0 0 .4rem 0; align-items:flex-start;">
        <input class="boon-provider-toggle" type="checkbox" data-provider-key="${escapeHtml(p.key)}" data-boon="${escapeHtml(boon)}" ${checked} />
        <span><span class="player-name">${escapeHtml(p.displayName || p.account)}</span>${account}<br><span class="muted-note">${escapeHtml(group)} · gen ${safeNumber(p.generation).toFixed(1)}${escapeHtml(inferred)}</span></span>
      </label>`;
    }).join("");
  }

  function renderBoonAssignments() {
    if (!els.boonEstimateSection || !els.boonEstimateBody) return;
    const assignments = state.boonAssignments || [];

    if (!assignments.length) {
      els.boonEstimateSection.hidden = true;
      els.boonEstimateBody.innerHTML = "";
      if (els.boonEstimateCount) els.boonEstimateCount.textContent = "";
      return;
    }

    els.boonEstimateSection.hidden = false;
    if (els.boonEstimateCount) {
      const totalSelected = assignments.reduce((sum, a) => sum + safeNumber(a.selectedCount), 0);
      els.boonEstimateCount.textContent = `${assignments.length} ambiguous log${assignments.length === 1 ? "" : "s"} · ${totalSelected} role selections counted`;
    }

    els.boonEstimateBody.innerHTML = assignments.map((assignment) => {
      return `<tr>
        <td><a class="encounter-link" href="${escapeHtml(assignment.url)}" target="_blank" rel="noopener noreferrer">${escapeHtml(assignment.fightName)} ↗</a></td>
        <td>${numberFmt.format(assignment.candidateCount)}</td>
        <td>${providerOptions(assignment.quickness, "Quickness")}</td>
        <td>${providerOptions(assignment.alacrity, "Alacrity")}</td>
        <td><strong>${numberFmt.format(assignment.selectedCount)}</strong> / ${numberFmt.format(assignment.totalRoleCandidates)} counted<br><span class="muted-note">Unticked names do not count as that boon provider for this log.</span></td>
      </tr>`;
    }).join("");
  }
  ...
  function installWorker() {
    ...
    state.worker = new Worker("worker-v6.js?v=1");
    ...
    state.worker.onmessage = (event) => {
      ...
      if (msg.type === "result") {
        ...
        state.reviewPoints = Array.isArray(msg.reviewPoints) ? msg.reviewPoints : [];
        state.boonAssignments = Array.isArray(msg.boonAssignments) ? msg.boonAssignments : [];
        renderAll();
        setBusy(false);
        const reviewText = state.reviewPoints.length ? ` · ${state.reviewPoints.length} suspicious datapoint${state.reviewPoints.length === 1 ? "" : "s"} to review` : "";
        const boonText = state.boonAssignments.length ? ` · ${state.boonAssignments.length} boon assignment${state.boonAssignments.length === 1 ? "" : "s"} reviewable` : "";
        setStatus(`Done: ${state.logs.length} usable logs, ${state.players.length} players${state.errors.length ? `, ${state.errors.length} failed` : ""}${reviewText}${boonText}.`, "success");
  ...
  function recalculate() {
    ...
    setStatus("Recalculating with your review and boon-role choices…", "loading");
    state.worker.postMessage({
      type: "recalculate",
      includeWipes: els.includeWipes.checked,
      excludedPointKeys: [...state.excludedPointKeys],
      providerSelections: [...state.providerSelections.entries()].map(([key, selected]) => ({ key, selected })),
    });
  }
  ...
  function analyze() {
    ...
    state.excludedPointKeys = new Set();
    state.providerSelections = new Map();
    state.reviewPoints = [];
    state.boonAssignments = [];
    installWorker();
    setBusy(true);
  ...
  function loadDemo() {
    const players = [
      ...
      { account: "Hammer.5555", displayName: "Bonk Department", profession: "Scrapper", encounters: 3, dps: 22100, deaths: 1, downs: 1, breakbar: 8340, resurrects: 2, mechanicScorePerLog: 1.3, distance: 96, distanceWeight: 1, boonProvider: true, quicknessUptime: 87.4, quicknessProviderLogs: 3 },
      { account: "Medic.1234", displayName: "Definitely A Healer", profession: "Druid", encounters: 3, dps: 6500, deaths: 0, downs: 1, breakbar: 1920, resurrects: 18, mechanicScorePerLog: 0, distance: 112, distanceWeight: 1, boonProvider: true, alacrityUptime: 89.2, alacrityProviderLogs: 3 },
    ];
    ...
    state.awards = [
      ...
      { icon: "⏩", title: "Quickness Outage", winner: players[2], stat: "87.4% average subgroup Quickness · provider average 94.6% · 3 included Quickness logs", roast: "Lowest included Quickness uptime among estimated providers — 7.2 percentage points below the provider average." },
    ];

    state.reviewPoints = [{
      key: "dps|Oops.7777|demo-vg",
      account: "Oops.7777",
      displayName: "Red Circle Enjoyer",
      metricLabel: "DPS",
      direction: "low",
      fightName: "Vale Guardian",
      url: "https://dps.report/demo-vg",
      valueText: "12,100",
      usualText: "28,900",
      gapText: "−16,800",
      excluded: false,
    ...
    state.boonAssignments = [{
      ...
      candidateCount: 5,
      selectedCount: 2,
      totalRoleCandidates: 4,
      quickness: [
        { key: "Quickness|Hammer.5555|https://dps.report/demo-vg", displayName: "Bonk Department", account: "Hammer.5555", group: 1, generation: 91.2, inferred: true, selected: true },
        { key: "Quickness|Random.1234|https://dps.report/demo-vg", displayName: "Incidental Boons", account: "Random.1234", group: 1, generation: 7.1, inferred: false, selected: false },
      ],
      alacrity: [
        { key: "Alacrity|Medic.1234|https://dps.report/demo-vg", displayName: "Definitely A Healer", account: "Medic.1234", group: 1, generation: 95.1, inferred: true, selected: true },
        { key: "Alacrity|RandomTwo.1234|https://dps.report/demo-vg", displayName: "Probably Incidental", account: "RandomTwo.1234", group: 1, generation: 6.4, inferred: false, selected: false },
      ],
    }];
  ...
  if (els.reviewBody) {
    els.reviewBody.addEventListener("change", (event) => {
      ...
      if (!(target instanceof HTMLInputElement) || !target.classList.contains("review-point-toggle")) return;
      const key = target.dataset.pointKey;
      if (!key) return;
      if (target.checked) state.excludedPointKeys.delete(key);
      else state.excludedPointKeys.add(key);
      recalculate();
    });
  }

  if (els.boonEstimateBody) {
    els.boonEstimateBody.addEventListener("change", (event) => {
      const target = event.target;
      if (!(target instanceof HTMLInputElement) || !target.classList.contains("boon-provider-toggle")) return;
      const key = target.dataset.providerKey;
      if (!key) return;
      state.providerSelections.set(key, target.checked);
      recalculate();
    });
  }
