...
function buildAwards(players) {
  ...
  if (strips) awards.push(award(
    ...
    "Reposessed the most enemy boons. Apparently none of those buffs had completed their payment plan."
  ));
  ...
  for (const p of players) {
    for (const [name, count] of Object.entries(p.mechanicNames || {})) {
      ...
      if (!specific || candidate.count > specific.count || (candidate.count === specific.count && candidate.rate > specific.rate)) specific = candidate;
    }
  }
  ...
  async function runner() {
    while (true) {
      ...
      try {
        ...
        cachedErrors.push({ url, message: error?.name === "AbortError" ? "Timed out" : (error?.message || "Unknown error") });
      } finally {
        done += 1;
        self.postMessage({ type: "progress", done, total: urls.length, loaded: cachedLogs.length, failed: cachedErrors.length, current: reportPath(url) });
      }
    }
  }
