(() => {
  ...
  function providerNames(items) {
    ...
    return items.map((p) => {
      ...
      return `${escapeHtml(p.displayName || p.account)} (${group}, gen ${safeNumber(p.generation).toFixed(1)})`;
    }).join("<br>");
  }

  function renderBoonEstimates() {
    if (!els.boonEstimateSection || !els.boonEstimateBody) return;
    const estimates = state.boonEstimates || [];
    ...
    if (els.boonEstimateCount) els.boonEstimateCount.textContent = `${estimates.length} ambiguous log${estimates.length === 1 ? "" : "s"}`;

    els.boonEstimateBody.innerHTML = estimates.map((estimate) => {
      const rejected = Array.isArray(estimate.rejected) && estimate.rejected.length
        ? estimate.rejected.map((p) => `${escapeHtml(p.displayName || p.account)} (Q ${safeNumber(p.quicknessGeneration).toFixed(1)}, A ${safeNumber(p.alacrityGeneration).toFixed(1)})`).join("<br>")
        : "—";
      return `<tr>
        <td><a class="encounter-link" href="${escapeHtml(estimate.url)}" target="_blank" rel="noopener noreferrer">${escapeHtml(estimate.fightName)} ↗</a></td>
        <td>${numberFmt.format(estimate.candidateCount)}</td>
        <td>${providerNames(estimate.quickness)}</td>
        <td>${providerNames(estimate.alacrity)}</td>
        <td>${rejected}</td>
      </tr>`;
    }).join("");
  }
  ...
  function installWorker() {
    ...
    state.worker = new Worker("worker-v5.js?v=1");
    ...
    state.worker.onmessage = (event) => {
      ...
      if (msg.type === "result") {
        ...
        state.boonEstimates = Array.isArray(msg.boonEstimates) ? msg.boonEstimates : [];
        renderAll();
        setBusy(false);
        ...
        const boonText = state.boonEstimates.length ? ` · ${state.boonEstimates.length} ambiguous boon log${state.boonEstimates.length === 1 ? "" : "s"} estimated` : "";
  ...
  function recalculate() {
    ...
    setStatus("Recalculating with your review choices…", "loading");
  ...
  function loadDemo() {
    ...
    state.boonEstimates = [{
      ...
      fightName: "Vale Guardian",
      candidateCount: 6,
      quickness: [{ displayName: "Bonk Department", account: "Hammer.5555", group: 1, generation: 91.2 }],
      alacrity: [{ displayName: "Definitely A Healer", account: "Medic.1234", group: 1, generation: 95.1 }],
      rejected: [{ displayName: "Incidental Boons", account: "Random.1234", quicknessGeneration: 7.1, alacrityGeneration: 0 }],
    }];
    state.logs = [
